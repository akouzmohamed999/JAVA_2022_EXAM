...
public class EcoleTechnomakerService {
    ...
    public Map<Enseignant, List<Formation>> getFormationsParEnseignant(EcoleTechnomaker ecoleTechnomaker) {
        return ecoleTechnomaker.getFormations().stream().collect(Collectors.groupingBy(formation ->
                formation.getEnseignant()));
    ...
    public Map<String, Long> getRepresentationEcole(EcoleTechnomaker ecoleTechnomaker) {
        return ecoleTechnomaker.getEtudiants().stream().collect(Collectors.groupingBy(etudiant->
                etudiant.getEcole(),Collectors.counting()));
    }
    ...
    public Map<Enseignant, Map<Boolean, Integer>> getFormationSommeVHPartitionParEnseignant(EcoleTechnomaker ecoleTechnomaker) {
        //if(ecoleTechnomaker.getFormations().)
        return ecoleTechnomaker.getFormations().stream().distinct().collect(Collectors.groupingBy(Formation::getEnseignant,
                Collectors.partitioningBy(formation -> formation.getEnseignant().getAnciennete()>1,Collectors.reducing(
                        0,Formation::getVolumeHoraire,(x,y)->x+y)
                )));
    }
