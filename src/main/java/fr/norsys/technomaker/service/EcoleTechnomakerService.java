...
public class EcoleTechnomakerService {
    ...
    public int calculerVolumeHoraire(EcoleTechnomaker ecoleTechnomaker) {
        return ecoleTechnomaker.getFormations().stream()
                .collect(Collectors
                        .reducing(0,formation -> formation.getVolumeHoraire(),(v,v2)->v+v2));
    }
    ...
    public Map<Enseignant, Map<Boolean, Integer>> getFormationSommeVHPartitionParEnseignant(EcoleTechnomaker ecoleTechnomaker) {
       /* return ecoleTechnomaker.getFormations().stream()
                .collect(Collectors.groupingBy(formation-> formation.getEnseignant()
                        ,Collectors.partitioningBy((f,f1)->{
                            int nb=0;
                            if(f.getEnseignant().getNom().equals(f1.getEnseignant().getNom())){
                                nb+=1;
                            }
                            return nb;
                            } >1
                                ,Collectors.reducing(0,formation -> formation.getVolumeHoraire(),(v,v2)->v+v2)
                        )
                        )
                );*/
        return null;
