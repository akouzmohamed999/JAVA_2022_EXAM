...
public class EcoleTechnomakerService {
    ...
    public Enseignant getEnseignantFromFormation(Formation formation){
        return Optional.ofNullable(formation).map(enseigant -> enseigant.getEnseignant()).orElseThrow(NoSuchElementException::new);
    }
    ...
    public Enseignant getEnseignantFromFormation(Formation formation1, Formation formation2){
       ...
       return Optional.ofNullable(formation1).or(() -> Optional.ofNullable(formation2)).map(enseigant -> enseigant.getEnseignant())
    ...
    public Map<Enseignant, List<Formation>> getFormationsParEnseignant(EcoleTechnomaker ecoleTechnomaker) {
        return ecoleTechnomaker.getFormations().stream().collect(Collectors.groupingBy(
        		ensei -> ensei.getEnseignant()));
    }
    ...
    public int calculerVolumeHoraire(EcoleTechnomaker ecoleTechnomaker) {
    	return ecoleTechnomaker.getFormations().stream().mapToInt(form -> form.getVolumeHoraire()).sum();
    }
    ...
    public Map<String, Long> getRepresentationEcole(EcoleTechnomaker ecoleTechnomaker) {
         return ecoleTechnomaker.getEtudiants().stream().collect(Collectors.groupingBy(ecole -> ecole.getEcole(),
        		 Collectors.counting()));
    }
    ...
    public Map<Enseignant, Map<Boolean, Integer>> getFormationSommeVHPartitionParEnseignant(EcoleTechnomaker ecoleTechnomaker) {
        return ecoleTechnomaker.getFormations().stream().collect(Collectors.groupingBy(
        		ensei -> ensei.getEnseignant(),
        		Collectors.groupingBy(Collectors.partitioningBy(ensei -> ensei.getEnseignant().getAnciennete() > 1,
        		Collectors.reducing(0, ensei -> ensei., (num1,num2)-> { //
                 return num1+num2;
                 		})	
        		))));
    }
